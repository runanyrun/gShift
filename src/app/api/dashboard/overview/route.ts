...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const payload = await controller.getOverviewForCurrentUser(authUserId);
    return applyResponseCookies(authResponse, NextResponse.json(payload, { status: 200 }));
  } catch (error) {
    return applyResponseCookies(authResponse, toApiErrorResponse(error, "Failed to load dashboard overview."));
  }
}
