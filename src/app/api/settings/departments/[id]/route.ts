...
export async function PATCH(
  ...
  try {
    ...
    const data = await controller.updateDictionary("departments", id, body);
    return applyResponseCookies(authResponse, NextResponse.json({ ok: true, data }, { status: 200 }));
  } catch (error) {
    return applyResponseCookies(authResponse, toApiErrorResponse(error, "Failed to update department."));
  }
}
