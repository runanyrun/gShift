...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const data = await controller.listDictionary("locations");
    return applyResponseCookies(authResponse, NextResponse.json({ ok: true, data }, { status: 200 }));
  } catch (error) {
    return applyResponseCookies(authResponse, toApiErrorResponse(error, "Failed to list locations."));
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const controller = new EmployeesController(supabase);
    const data = await controller.createDictionary("locations", body);
    return applyResponseCookies(authResponse, NextResponse.json({ ok: true, data }, { status: 201 }));
  } catch (error) {
    return applyResponseCookies(authResponse, toApiErrorResponse(error, "Failed to create location."));
  }
}
