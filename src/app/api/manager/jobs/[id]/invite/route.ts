...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data, error } = await auth.supabase.rpc("invite_applicant" as never, {
      p_job_id: params.id,
      p_worker_id: body.worker_id,
      p_note: body.note ?? null,
    } as never);
    if (error) {
      throw new Error(`Failed to invite applicant: ${error.message}`);
    }

    return applyResponseCookies(authResponse, NextResponse.json({ ok: true, data }, { status: 200 }));
  } catch (error) {
    return applyResponseCookies(authResponse, toApiErrorResponse(error, "Failed to invite applicant."));
  }
}
