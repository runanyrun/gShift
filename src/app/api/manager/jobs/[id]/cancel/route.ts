...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data, error } = await auth.supabase.rpc("cancel_job" as never, {
      p_job_id: params.id,
      p_reason: body.reason,
    } as never);

    if (error) {
      throw new Error(`Failed to cancel manager job: ${error.message}`);
    }

    return applyResponseCookies(authResponse, NextResponse.json({ ok: true, data }, { status: 200 }));
  } catch (error) {
    return applyResponseCookies(authResponse, toApiErrorResponse(error, "Failed to cancel manager job."));
  }
}
