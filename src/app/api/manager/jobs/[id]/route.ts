...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { data, error } = await auth.supabase.rpc("get_job" as never, {
      p_job_id: params.id,
    } as never);

    if (error) {
      throw new Error(`Failed to load manager job detail: ${error.message}`);
    }
    ...
    return applyResponseCookies(authResponse, toApiErrorResponse(error, "Failed to load manager job detail."));
  }
}
